...
/// <summary>
/// Phase 15 L2-PROJECT-02 — verifies the three flat L2 key formats produced by
/// <see cref="RedisProjectionKeys"/>. The scheme is a single prefix + GUID(s) with NO
/// type discriminator: Root and Processor are byte-identical for the same prefix+Guid
/// (D-02 — disambiguated only by GUID namespace). GUIDs render in the default "D" format
/// (hyphenated), NOT "N". <see cref="RedisProjectionKeys"/> is internal — these tests rely
/// on the existing <c>InternalsVisibleTo("BaseApi.Tests")</c> on BaseApi.Service.
/// </summary>
[Trait("Phase", "15")]
public sealed class RedisProjectionKeysTests
{
    ...
    {
        var key = RedisProjectionKeys.Root("skp:", Workflow);
    ...
    {
        var key = RedisProjectionKeys.Step("skp:", Workflow, Step);
        Assert.Equal(
    ...
    {
        var key = RedisProjectionKeys.Processor("skp:", Processor);
    ...
    public void Root_And_Processor_Are_ByteIdentical_For_Same_Prefix_And_Guid()
    {
        ...
        var root = RedisProjectionKeys.Root("skp:", sharedGuid);
        var processor = RedisProjectionKeys.Processor("skp:", sharedGuid);
    ...
    public void PerClass_Test_Prefix_Composes_The_Same_Way()
    {
        ...
        Assert.Equal(
            "test:cls-abc:11111111-1111-1111-1111-111111111111",
            RedisProjectionKeys.Root(prefix, Workflow));
        Assert.Equal(
            "test:cls-abc:11111111-1111-1111-1111-111111111111:22222222-2222-2222-2222-222222222222",
            RedisProjectionKeys.Step(prefix, Workflow, Step));
        Assert.Equal(
            "test:cls-abc:33333333-3333-3333-3333-333333333333",
            RedisProjectionKeys.Processor(prefix, Processor));
    }
}
