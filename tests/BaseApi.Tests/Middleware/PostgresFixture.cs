...
///
/// <para>
/// <b>Verbatim Plan 03-02 lift</b> (Plan 04-02 PATTERNS.md lines 287-349 + CONTEXT.md D-14):
/// the only change versus tests/BaseApi.Tests/Persistence/PostgresFixture.cs is the
/// namespace + the database-name prefix (`stepsdb_test_mw_` instead of
/// `stepsdb_test_`) so cleanup-snapshot diffs are attributable to Plan 04-02
/// rather than Phase 3. The cleanup discipline (ClearAllPools + DROP DATABASE
/// WITH FORCE) is the SAME contract — preserves cross-phase D-15 invariant.
/// </para>
///
/// <para>
...
/// fails under heavy parallelism, the leaked DB surfaces in the Plan 04-02 SUMMARY
/// `psql \l` snapshot diff (manual cleanup acceptable per RESEARCH.md A4).
/// </para>
/// </summary>
public sealed class PostgresFixture : IAsyncLifetime
{
    public string DatabaseName { get; } = $"stepsdb_test_mw_{Guid.NewGuid():N}";
