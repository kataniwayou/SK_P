...
| CONSOLE-HEALTH-03, CONSOLE-HEALTH-04 | `/health/startup` flips Healthy after host init; `/health/ready` reflects bus state | integration | Boot fixture; assert `/health/startup` = 200 after `StartAsync`; negative variant removing `StartupCompletionService` → 503 (mirror `HealthNoStartupCompletionFixture`); assert `/health/ready` tag wiring | `ConsoleStartupGateTests.cs` | ❌ W0 |
