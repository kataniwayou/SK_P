...
The L2 Redis projection gains a two-level SET-based membership index — a parent index enumerating all active workflow IDs, and per-workflow member step-id sets — the key prefix becomes a hardcoded compile-time constant, and the orchestration Start flow stops *creating* processor L2 entries and instead *validates* each participating processor's self-registered entry for existence and timestamp-based liveness, failing with the same 422 contract as other Start-time validations.
...
2. **L2IDX-02 — Per-workflow member step-id set**: Each active workflow's member step IDs are enumerable as a Redis SET, with no graph traversal required.
   - Current: A workflow's step keys are discoverable only by BFS over `EntryStepIds`/`NextStepIds`.
   - Target: The writer maintains a per-workflow SET of that workflow's member step IDs; Cron/JobId/Liveness remain in the workflow root value. Step *data* stays in the existing per-step child keys.
   - Acceptance: After Start, `SMEMBERS` of a workflow's member set returns exactly the step IDs of that workflow's graph; no `NextStepIds` traversal is needed to enumerate them.

3. **L2PREFIX-01 — Hardcoded prefix constant**: The L2 key prefix is a compile-time constant, not configuration.
...
4. **PROC-NOCREATE-01 — Writer stops creating processor L2 entries**: The Start flow no longer writes `ProcessorProjection` keys to L2.
...
5. **PROC-LIVE-01 — Processor existence + liveness validation at Start**: The Start flow validates that every participating processor has a live self-registered L2 entry, failing identically to other Start-time validations.
...
6. **PROC-EDGE-01 — Edge-schema validation preserved**: Processor edge-schema validation behavior is unchanged.
...
- Per-workflow member step-id Redis SET, maintained by the writer (L2IDX-02).
...
- **Introduces the Redis SET type** to a keyspace that is `StringSet`-only today; SET membership ops (`SADD`/`SMEMBERS`/`SREM`) are the chosen primitives for the index and member lists.
- **Partially lifts the v3.4.0 "no Orchestrator Redis writes / new keyspaces" lock** — but only for the **writer-side** parent-index `SADD` and member-set maintenance; the orchestrator process still performs no L2 writes in this phase (orchestrator writes are Phase 23).
...
- [ ] After Start, `SMEMBERS` of a workflow's member set returns exactly that workflow's step IDs, with no `NextStepIds` traversal required.
...
|--------------------|-------|------|--------|--------------------------------------------------------------|
| Goal Clarity       | 0.88  | 0.75 | ✓      | Two-level SET index + const prefix + validate-not-create     |
