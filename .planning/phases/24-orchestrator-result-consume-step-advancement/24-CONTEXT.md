...
- **Per-workflowId safe L1 eviction (`FUTURE-STOP-EVICTION`)** — the user's stated next problem: a mechanism to verify that no processor messages for a given `workflowId` remain in flight on the bus, after which it is safe to clear that workflow's L1 entry. Until built, stopped workflows linger in L1 (accepted; bounded population). NOT this phase.
