...
- [ ] **ORCH-ADVANCE-01**: On a result the orchestrator reads the completed step's L1 projection, enumerates its `NextStepIds`, reads each next step's L1 projection, and selects a next step iff `nextStep.EntryCondition == (int)outcome` OR `nextStep.EntryCondition == Always` (`Never` never advances). The result path performs NO L2/Redis read.
