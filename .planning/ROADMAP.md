...
**Success Criteria** (what must be TRUE):
  ...
  2. Start-twice with same WorkflowIds confirms L2 keys reflect the second write (idempotent overwrite); a concurrent-Start regression test (two parallel HTTP requests) documents the last-write-wins / partial-state-interleave behavior; Stop-after-Start returns 204 and the workflow's root + reachable per-step keys are removed (GET-and-follow cleanup) while per-processor keys remain intact (TTL'd); Stop-without-prior-Start returns 422 with the full missing list and deletes nothing; a repeated Stop of an already-cleaned workflow re-fails the gate → 422 (non-idempotent).
