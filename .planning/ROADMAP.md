...
- [ ] **Phase 38: Uniform `service_name` + Instance Labels Across All Metrics** — `service_name={name}_{version}` + `service_instance_id` on every metric series (runtime/HTTP/business); processor name+version sourced from the DB (not appsettings); logs `service.name` unchanged; Prometheus query consumers updated. (MLBL-01..05)
- [ ] **Phase 39: Keeper Observability + Real-Stack E2E + Close Gate** — `Keeper` meter + counters/histograms; E2E proving recover-both-paths + give-up; 3×GREEN triple-SHA net-zero close gate (both DLQs + scratch-key scan-clean). (KMET-01/02/03, TEST-01/02/03)
