...
| ----- | ---- | ---- | ------------ | ---------------- |
| 43 | Message Contracts & L2 Key Reshape | The six-id contracts (no `H`), the `Guid.Empty` source sentinel, the five Keeper-state messages, and the two L2 key schemes (GUID data key no-TTL + composite backup key TTL 2d) exist as the shared vocabulary everything else builds on. RETIRE-01 (`H`/`flag[H]`/CAS) and RETIRE-02 (content-addressing/result manifest/N×M fan-out) are removed HERE — coupled to the field/type reshape (compile-forced per D-01), not deferred. | MSG-01, MSG-02, MSG-03, RETIRE-01, RETIRE-02 | 4 |
