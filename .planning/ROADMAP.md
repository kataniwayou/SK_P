...
**Success Criteria** (what must be TRUE):
  ...
  2. On consuming Start/Stop, the Orchestrator opens the correlated log scope from the **message envelope `CorrelationId`** (minted fresh at publish) and reads the Redis L2 root per `WorkflowId` for existence/payload (the absent-from-L2 case is the MSG-ACK-01 business-failure path); it logs up to the "scheduler job start" seam — performing no Redis writes and no Quartz scheduling. The HTTP `X-Correlation-Id` is not stored in or read from the L2 root; correlation is per-stage with a handoff at the publish boundary.
