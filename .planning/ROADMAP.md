...
- 🚧 **v9.0.0 Canonical Recovery — Orchestrator Alignment** — Phases 70-71 (started 2026-06-16) — extend the Phase-69 canonical recovery-spec alignment from the processor to the orchestrator result-consume path (same messageId-indexed forward/recovery/keeper pipeline; reverses Phase 24.1 L1-only TypedResultConsumer, re-introducing L2 to the result path) + a small processor INJECT cleanup making DELETE the only key-deleting keeper state; 10 KINJ/ORCV reqs; canonical pattern ProcessorPipeline.cs + docs/design/processor-keeper-recovery-spec.md §3–§8

## 🚧 v9.0.0 Canonical Recovery — Orchestrator Alignment (In Progress — started 2026-06-16)

**Milestone Goal:** Extend the Phase-69 canonical recovery-spec alignment from the **processor** to the **orchestrator's result-consume path**, giving it the same `messageId`-indexed forward / recovery / keeper pipeline the processor already runs (`src/BaseProcessor.Core/Processing/ProcessorPipeline.cs` + [`docs/design/processor-keeper-recovery-spec.md`](../docs/design/processor-keeper-recovery-spec.md) §3–§8) — so an orchestrator crash/redelivery re-emits **stable persisted entryIds idempotently** (at-least-once delivery + stable re-emission, **NOT** message-level dedup) instead of relying on Phase 24.1's L1-only straight-through `TypedResultConsumer` (which this milestone **reverses**, re-introducing L2 to the result path). Plus a small **processor INJECT cleanup** that makes the keeper delete-invariant uniform across both consoles: **DELETE is the only keeper state that deletes keys** — INJECT and REINJECT are non-destructive (write + send only).

**Idempotency model (both consoles):** the inbound message's broker `messageId` is the idempotency/branch key (`exist L2[messageId]` gate, slot array). A redelivery re-enters RECOVERY and re-sends the **persisted** entryIds (stable; retired slots skipped); the atomic two-key delete closes the index-gone/input-alive reprocess window. **Delete invariant:** keys are deleted ONLY in the cleanup tail (a forward exit where no item escalated, or the end of a recovery pass), completed out-of-band by the DELETE keeper state on exhaust — INJECT/REINJECT never delete.

**Source of truth:** This milestone's design conversation (2026-06-16), confirmed point-by-point — orchestrator forward/recovery mirror, atomic index+data copy, atomic two-key delete, INJECT non-destructive (no source delete), origin-split keeper contracts, and the delete invariant. Requirements: [REQUIREMENTS.md](REQUIREMENTS.md) (10 reqs across KINJ + ORCV). Phases continue at **70**.

**Build order (locked, dependency-driven):** **70** Processor INJECT Cleanup (small — establishes the non-destructive-INJECT pattern Phase 71 mirrors) → **71** Orchestrator Recovery Pipeline (large — **depends on Phase 70**).

### Phases

- [x] **Phase 70: Processor INJECT Cleanup** — Make the keeper INJECT path non-destructive (write + send, no source delete); drop the vestigial `KeeperInject.DeleteEntryId` (+ `BuildInject`/golden tests); DELETE becomes the only key-deleting keeper state. ✅ 2026-06-16 (2/2 plans; hermetic build 0-warning Release+Debug; full-suite-green half is live-stack/operator-gated)
- [ ] **Phase 71: Orchestrator Recovery Pipeline** — Give the orchestrator's result-consume path the `messageId`-indexed forward/recovery/keeper pipeline (exist-gate → FORWARD atomic index+data copy / RECOVERY 3-way per-slot re-emit / keeper escalation), full-dispatch-tuple slots, origin-split keeper contracts (`Processor*`/`Orchestrator*`, shared `KeeperDelete`) on the existing `keeper-recovery` endpoint. Reverses Phase 24.1's L1-only `TypedResultConsumer`.

### Phase Details

#### Phase 70: Processor INJECT Cleanup
**Goal**: The processor keeper INJECT path becomes **non-destructive** — `InjectConsumer` writes the data key and sends the result but deletes NO key (the trailing `delete L2[DeleteEntryId]` is removed). The now-vestigial `KeeperInject.DeleteEntryId` field is dropped from the contract, `ProcessorPipeline.BuildInject` no longer supplies it, and `InjectConsumerFacts` + the Phase-50 golden tests move to the new shape. The result is the uniform delete invariant that Phase 71 mirrors: **DELETE is the only keeper state that deletes keys** — REINJECT and INJECT never delete. This establishes the non-destructive-INJECT pattern that the new orchestrator keeper contracts (Phase 71) copy.
**Depends on**: — (first phase of v9.0.0; builds on the shipped Phase-69 atomic-write processor pipeline + the v5.0.0 3-state keeper)
**Requirements**: KINJ-01, KINJ-02, KINJ-03
**Success Criteria** (what must be TRUE):
  1. `InjectConsumer.HandleAsync` performs exactly two effects — write `L2[EntryId]=Data` then send `StepCompleted` to `queue:orchestrator-result` — and issues **no** `KeyDelete*` (the former source-delete tail step at `InjectConsumer.cs:40` is gone); a fact asserts the consumer never calls a delete overload. (KINJ-01)
  2. `KeeperInject` no longer carries `DeleteEntryId` (the field is deleted from the contract), `ProcessorPipeline.BuildInject` constructs the contract without it, and a source/reflection scan finds no remaining `DeleteEntryId` reference on the INJECT path. (KINJ-02)
...
**Plans**: 2 plans
  ...
  - [x] 70-02-PLAN.md — Test reshape: new KeeperDeleteInvariantFacts (D-04/D-05), reflection guard + InjectConsumerFacts/PipelineForward/SC2 E2E updates (D-06..D-09), 0-warning dual-config build gate (D-10) ✅ 2026-06-16 (52c0780, b110f67, 2292651)

#### Phase 71: Orchestrator Recovery Pipeline
**Goal**: The orchestrator's result-consume path gains the same `messageId`-indexed forward/recovery/keeper pipeline the processor has (canonical pattern `ProcessorPipeline.cs` + spec §3–§8), **reversing Phase 24.1's L1-only `TypedResultConsumer`** by re-introducing L2 to the result path. On consume it evaluates the `exist L2[messageId]` gate once (messageId = the inbound result's broker message id): **absent → FORWARD**, **present → RECOVERY**, gate-op exhaustion → REINJECT (end round trip, no cleanup). FORWARD, per next step, does ONE atomic op (index-slot `HSET` + whole-hash `PEXPIRE` + copy `L2[origin entryId]→L2[new entryId]` with the data TTL), routes a write-exhaust to a single `OrchestratorInject` (no silent drop), sends `EntryStepDispatch` to `queue:{nextProcessorId}`, retires the slot, and runs the gated atomic two-key cleanup tail only if no item escalated. RECOVERY re-emits idempotently — per slot a 3-way classification (data exists → re-send; clean not-exist → drop, no retire; L2 fault → leave slot intact), tail REINJECTs on any fault else runs the atomic two-key delete; a redelivery re-sends the stable persisted entryIds and skips retired slots. Each slot carries the **full dispatch tuple** `(nextStepId, nextProcessorId, payload, newEntryId)` because orchestrator slots are heterogeneous (unlike the processor's homogeneous slots). Keeper contracts split by **origin** (route-by-type, no discriminator switch): `KeeperInject`/`KeeperReinject` rename to `ProcessorInject`/`ProcessorReinject`; `OrchestratorInject`/`OrchestratorReinject` are added; `KeeperDelete` stays shared. The two new consumers bind on the **same** `keeper-recovery` endpoint (same partitioner, health gate, exhaustion posture — no new queue). The delete invariant holds orchestrator-side: keys are deleted ONLY in the cleanup tail (completed out-of-band by DELETE on exhaust); `OrchestratorInject`/`OrchestratorReinject` never delete.
**Depends on**: Phase 70 (the non-destructive-INJECT pattern + the `DeleteEntryId`-free contract shape established in Phase 70 are mirrored by the new `Orchestrator*` keeper contracts; the rename `KeeperInject→ProcessorInject` builds on Phase 70's reduced contract)
**Requirements**: ORCV-01, ORCV-02, ORCV-03, ORCV-04, ORCV-05, ORCV-06, ORCV-07
**Success Criteria** (what must be TRUE):
  1. The orchestrator result-consume path gates on `exist L2[messageId]` (messageId = the inbound result's broker message id): absent runs FORWARD, present runs RECOVERY, and a gate L2-op exhaustion routes to REINJECT and ends the round trip with no cleanup — replacing the L1-only straight-through `TypedResultConsumer` (L2 is back on the result path). (ORCV-01)
  2. FORWARD, per next step, performs ONE atomic Redis op — index-slot `HSET` + whole-hash `PEXPIRE` + copy `L2[origin entryId]→L2[new entryId]` with the data TTL — then sends `EntryStepDispatch` to `queue:{nextProcessorId}` and retires the slot to `guid.empty`; an atomic-write exhaustion routes to a single `OrchestratorInject` (no silent drop), and each slot value carries the full dispatch tuple `(nextStepId, nextProcessorId, payload, newEntryId)` so RECOVERY can reconstruct the heterogeneous per-slot dispatch. (ORCV-02, ORCV-03, ORCV-04)
...
- [x] 69-02-PLAN.md -- Wave 2: gate the forward cleanup tail on no-escalation + EscalatedItem_SkipsCleanup fact
