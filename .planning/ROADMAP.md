...
**Success Criteria** (what must be TRUE):
  1. After a successful `POST /api/v1/orchestration/start`, `redis-cli GET {prefix}{workflowId:N}` returns the expected JSON shape — `{ entryStepIds[], cron, jobId, liveness{timestamp,interval,status}, correlationId }` — where `jobId` is a fresh `Guid.NewGuid()` per Start (NOT `Guid.Empty`) and `correlationId` matches the inbound `X-Correlation-Id` header.
  ...
  3. _(amended Phase 15, 2026-05-29)_ `POST /api/v1/orchestration/stop` EXISTS-gates the requested `{workflowId}` root keys: when ANY root is absent it returns 422 with the FULL missing-workflowIds list and performs NO deletion (the gate fails before any cleanup). When ALL roots exist it runs the per-workflow GET-and-follow cleanup — deletes each root key + its reachable per-step keys, NEVER per-processor keys — and returns 204. A repeated Stop of an already-cleaned workflow re-fails the gate → 422 (non-idempotent by design). Stop does NOT touch Postgres.
  ...
  5. _(amended Phase 15, 2026-05-29)_ `OpenTelemetry.Instrumentation.StackExchangeRedis` is NOT referenced anywhere in the solution; Redis ops appear in MEL logs with X-Correlation-Id flowing via AsyncLocal (verified by the `OrchestrationLogsE2ETests` E2E extending the Phase 11 `SchemasLogsE2ETests` pattern). `IServer.Keys()` and `KEYS` are forbidden in production code: Stop cleanup traverses by **targeted GET-and-follow** (`StringGetAsync` on the root then each reachable step), NOT by keyspace enumeration — only SCAN-based enumeration would be allowed if enumeration were ever needed (it is not).
**Plans**: 5 plans
...
- [x] 15-04-PLAN.md — OrchestrationService per-workflow Start loop + Redis Stop gate/cleanup + DI/controller wiring
- [x] 15-05-PLAN.md — OBSERV-REDIS-02 E2E + discipline guards + Phase 14 ValidationOrderFacts amend + REQUIREMENTS/ROADMAP reconcile
...
> NOTE (flagged by Plan 15-05): SC2 and SC5 above are now INVERTED by the 15-CONTEXT Stop amendments — post-Stop the root + per-step keys are GONE (deleted by GET-and-follow cleanup) while per-processor keys remain (TTL'd). The "Stop does NOT delete any L2 keys / post-Stop `SCAN` matches pre-Stop" assertions must be rewritten to "post-Stop root + per-step removed, processors intact." Phase 16 owns this SC edit + its own facts; the `redis-cli --scan` BEFORE=AFTER full-suite invariant still holds across a *balanced* suite (test cleanup zeroes residual `test:cls-*`).
**Plans**: TBD
