...
- [ ] **Phase 70: Processor INJECT Cleanup** — Make the keeper INJECT path non-destructive (write + send, no source delete); drop the vestigial `KeeperInject.DeleteEntryId` (+ `BuildInject`/golden tests); DELETE becomes the only key-deleting keeper state.
