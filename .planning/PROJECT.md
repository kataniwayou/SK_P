...
**Shipped:** v3.4.0 (BaseConsole + Orchestrator Messaging) — 2026-06-01 · [milestone archive](milestones/v3.4.0-ROADMAP.md)
**Previously:** v3.3.0 (Orchestration L3 → L1 → L2 Build Pipeline) — 2026-05-29 · [milestone archive](milestones/v3.3.0-ROADMAP.md)
...
## Current Milestone: v3.5.0 Processor Console — Self-Registration, Liveness & Execution Round-Trip
...
**Milestone v3.5.0 (Processor Console — Self-Registration, Liveness & Execution Round-Trip) — started 2026-06-01.** Building the reusable `BaseProcessor.Core` + first concrete `Processor.Sample`, the assembly-embedded SourceHash identity, the WebApi bus responders (`GetProcessorBySourceHash` / `GetSchemaDefinition`), processor liveness self-registration into Redis L2, and the live orchestrator→processor→orchestrator execution round-trip (consume `EntryStepDispatch` → L2 input → `abstract ProcessAsync` → L2 output + `ExecutionResult`). See `.planning/REQUIREMENTS.md` for scoped REQ-IDs; phase numbering continues from 24 (starts at **Phase 25**). **Phase 25 (Shared Contracts + WebApi Responders) complete 2026-06-01** — CONTRACT-01/02/03 + RPC-01/02/03 satisfied (13/13 must-haves verified, code review clean): `ProcessorProjection` relocated public into `Messaging.Contracts`, `L2ProjectionKeys.ExecutionData` + `LivenessStatus.Healthy` added, the six request/response records + `ProcessorQueues` constants landed, and the WebApi now hosts the `GetProcessorBySourceHash` / `GetSchemaDefinition` dual-response responders on explicit `ReceiveEndpoint`s — the Phase-19 publish-only firewall + Degraded health cap preserved. The processor (Phase 26+) now has a vocabulary to query. **Phase 26 (BaseProcessor.Core — Library, Identity & Liveness) complete 2026-06-01** — BPC-01/02/03 + IDENT-03/04 + RPC-04 + SCHEMA-01/02 + LIVE-01..06 + CONFIG-01 satisfied (15/15 reqs; verification 5/5 must-haves PASSED, code review 0 critical / 3 advisory warnings; *Processor* slice 32/32 GREEN): the bus-firewalled `BaseProcessor.Core` library on `BaseConsole.Core` shipped — `AddBaseProcessor` composition root + two-loop `ProcessorStartupOrchestrator` (Loop A identity-by-SourceHash, Loop B per-non-null-schema definition resolution, both via `exchange:`-scheme dual-response `IRequestClient` with unbounded retry + bounded backoff, boot-before-register tolerant, config schema never queried), the only-when-Healthy sliding-SET liveness heartbeat writing `skp:{id:D}` (reusing the frozen `ProcessorProjection`/`LivenessProjection` + `L2ProjectionKeys.Processor` + `LivenessStatus.Healthy` verbatim), and a closed writer→reader round-trip proving the written L2 value passes the unchanged v3.4.0 `ProcessorLivenessValidator` as live then goes stale past `interval×2`. The concrete `Processor.Sample` + assembly-embedded SourceHash + live execution round-trip remain for Phases 27–28.

v3.4.0 (BaseConsole + Orchestrator Messaging) shipped 2026-06-01 — phases 17–24 + 24.1, 70/70 requirements, 335/335 GREEN, audit PASSED, tagged `v3.4.0` (full record in `MILESTONES.md` + `milestones/v3.4.0-*`).
