...
- **Liveness heartbeat worker** — every `Interval`s write/refresh `skp:{processorId:D}` = `{ inputDefinition, outputDefinition, liveness{ timestamp, interval, status } }` with `Ttl` expiry; `status` = Unhealthy until identity + all required (non-null) definitions resolve, then Healthy.
