# Phase 39 close gate — v3.7.0 (triple-SHA)
# Keeper observability + real-stack E2E — keeper meter rebuild + both-DLQ net-zero closeout
# ---------------------------------------------------------------------------
# This v3.7.0 gate is a verbatim clone of scripts/phase-33-close.ps1 (the triple-SHA protocol),
...
#       * the Keeper L2-probe scratch keys skp:keeper:probe:* — written transiently during a probe
#         loop; the UNFILTERED --scan SHA already covers any leaked scratch key (no scan delta needed),
#         and they are TTL'd/cleaned by the E2E teardown.
...
#     NOTE (Pitfall 4): the name-SHA input stays `list_queues name` (NOT `name messages`) — folding
#     the message-depth column into the SHA would churn it every run. The both-DLQ depth==0 check
#     (DELTA 2 below) is a SEPARATE additive assertion that uses `list_queues name messages`.
#   - Phase 3 D-18 3-consecutive-GREEN cadence (full suite, no Category filter — RealStack E2E run live).
#
# NOTE (Phase 39 — keeper observability): the keeper recovery path's poison/data/flag/probe-scratch keys are
# all TTL'd/content-addressed and captured by the unfiltered --scan SHA + drained by the test's
# L2KeysToCleanup. The simpler proven triple-SHA protocol is the correct gate; the KeeperRecoveryE2ETests
# poison/probe keys are net-zeroed by the TEST's L2KeysToCleanup (not this gate).
#
# The net-zero discipline for the skp:data:*/skp:flag:*/skp:keeper:probe:* namespaces is enforced by the E2E
# teardown (the E2E tests register every namespace — plus every armed WRONGTYPE poison key — into
# L2KeysToCleanup, D-12); this gate's job is to SNAPSHOT-and-COMPARE the full unfiltered keyspace.
# There is deliberately NO destructive whole-db flush and NO prefix filter on the scan — widening to the
# full keyspace is what makes skp:flag:* part of the SHA. Likewise the both-DLQ depth==0 check (DELTA 2)
# is a snapshot-and-compare assertion: the net-zero DLQ drain stays in the E2E teardown (D-10), NOT this gate
# (NO gate-side purge_queue) — so a teardown regression surfaces here as depth>0, not a silent pass.
#
...
# This is the SAME idempotent row the E2E (KeeperRecoveryE2ETests / IdempotentExactlyOnceE2ETests) reuses,
...
#
# CONTRACT-CHANGE NOTE (v3.7.0): the wire contract is NOT backward-compatible (EntryId Guid->string + new
# H). The live stack MUST run the REBUILT contract-changed + metered images
# (docker compose up -d --build baseapi-service orchestrator processor-sample keeper) — a mixed-version
# deployment mis-deserializes the new contract, the embedded SourceHash must match the host build (Pitfall 5)
# or the liveness gate false-passes/times out, AND a stale Phase-34 keeper image has no Keeper meter (Pitfall
# 3) so the keeper_* series come back empty and the E2E scrape asserts go RED. The rebuild set is therefore
# `baseapi-service orchestrator processor-sample keeper` (DELTA 1).
#
# The full v3.7.0 stack MUST be up healthy (postgres, redis, rabbitmq, otel-collector,
# elasticsearch, prometheus, orchestrator, processor-sample, baseapi-service, keeper).
#
# Usage:
#   pwsh -File scripts/phase-39-close.ps1
#
# Exit codes:
#   0  — both build configs zero-warning, all three runs GREEN, all three SHA-256 invariants held,
#        both DLQs (keeper-dlq + skp-dlq-1) depth==0
...
try {
    Write-Host "Phase 39 close gate (triple-SHA) starting from $repoRoot" -ForegroundColor Cyan
    ...
    try {
        ...
        $body = @{
            ...
            version        = '3.7.0'
            description    = 'Phase 39 close-gate steady-state Processor.Sample row (genuine embedded hash, schema-less).'
    ...
    if (-not $procHealthy) {
        ...
        Write-Host "  (or the container predates the v3.7.0 wire contract — rebuild it: docker compose up -d --build processor-sample)." -ForegroundColor Red
        exit 2
    }
    Write-Host "  processor-sample is healthy (steady-state liveness key skp:$($procId.ToString().ToLower()) live)." -ForegroundColor Green

    # ---- Pre-flight: compose stack healthy (ONE canonical v3.7.0 list, processor-sample + keeper included) ----
    ...
    Write-Host "  Observed Phase 39 fact count: $($distinctPassed[0])." -ForegroundColor Gray

    # ---- Settle-drain the transient TTL-bounded namespaces (NET-ZERO-31) ----
    # The skp:flag:{64hex} / skp:data:{64hex} / skp:keeper:probe:* keys are per-fire-UNIQUE (H embeds the
    # per-fire correlationId) and TTL-bounded (keepTtl, 300s). Every RealStack E2E test now STOPS its workflow
    # in teardown, so no self-rescheduled cron fire keeps minting fresh names after the runs finish. Wait for
    ...
    if ($beforeRedisHash -ne $afterRedisHash) {
        ...
        Write-Host "  skp:data:{64hex} OR a leaked effect-first dedup flag skp:flag:{64hex} OR a leaked Keeper" -ForegroundColor Red
        Write-Host "  L2-probe scratch key skp:keeper:probe:* (the E2E net-zero teardown should drain ALL of" -ForegroundColor Red
        Write-Host "  these namespaces + every armed WRONGTYPE poison — D-12), or a churned" -ForegroundColor Red
    ...
    # ---- Both-DLQ depth==0 assertion (DELTA 2 — additive, SEPARATE from the name-SHA per Pitfall 4) ----
    # Net-zero closeout for BOTH dead-letter queues: keeper-dlq (DLQ-2, KeeperQueues.DeadLetter — terminal
    # probe give-up, NO TTL) AND skp-dlq-1 (DLQ-1, ConsolidatedErrorTransportFilter.Dlq1 — TTL'd forensic sink).
    # This uses `list_queues name messages` to read the depth column, kept OUT of the name-SHA above (the SHA
    # input stays `list_queues name`) so depth churn cannot break the name invariant. The net-zero drain stays
    # in the E2E teardown (D-10) — NO gate-side purge_queue — so a teardown regression surfaces here as depth>0.
    ...
    if (-not $allGood) {
        Write-Host "Phase 39 close gate FAILED. Resolve violations and re-run." -ForegroundColor Red
        exit 1
    }

    Write-Host "Phase 39 close gate PASSED." -ForegroundColor Green
    ...
    Write-Host "  DLQ depths: keeper-dlq=0, skp-dlq-1=0" -ForegroundColor Green
    Write-Host "Operator: append these three SHA values + the Passed count + both DLQ depths to .planning/STATE.md Phase 39 close entry." -ForegroundColor Cyan
