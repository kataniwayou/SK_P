# Phase 16 close gate — v3.3.0
# Idempotency + concurrency + L1 cleanup + 3-GREEN closeout
# ---------------------------------------------------------------------------
# Honors:
...
#     6th consecutive phase to record)
...
#
# Usage:
#   pwsh -File scripts/phase-16-close.ps1
#
# Exit codes:
...
try {
    Write-Host "Phase 16 close gate starting from $repoRoot" -ForegroundColor Cyan
    ...
    # ---- Negative schema-migration assertion (no new EF migrations from Phase 16) ----
    ...
    if (-not [string]::IsNullOrWhiteSpace($newMigrations)) {
        ...
        Write-Host "No EF migrations generated by Phase 16 (negative schema-push assertion HELD)." -ForegroundColor Green
    }
    ...
    if (-not $allGood) {
        Write-Host "Phase 16 close gate FAILED. Resolve violations and re-run." -ForegroundColor Red
        exit 1
    }

    Write-Host "Phase 16 close gate PASSED." -ForegroundColor Green
    ...
    Write-Host "Operator: append these values to .planning/STATE.md Phase 16 P05 close entry." -ForegroundColor Cyan
