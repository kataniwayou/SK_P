# Phase 49 close gate — v4.0.0 (triple-SHA)
# Processor Pre/In/Post-Process + Keeper Recovery Redesign — live-proof + net-zero closeout
# ---------------------------------------------------------------------------
# This v4.0.0 gate is a verbatim clone of scripts/phase-39-close.ps1 (the proven triple-SHA protocol),
# updated ONLY for the v4 namespaces / service list / single-DLQ (D-06/D-07). The triple-SHA protocol
# (idempotent steady-state Processor-row seed, compose-health pre-flight, BOTH-config 0-warning build
# gate, 3-consecutive-GREEN identical-fact-count cadence, settle-drain, triple-SHA BEFORE==AFTER,
# separate DLQ depth==0) is IDENTICAL to phase-39.
#
...
#     The scan is UNFILTERED (ALL keys), so the v4 key families are captured automatically in the
#     BEFORE/AFTER SHA — NO new prefix filter excludes them (D-07):
#       * the GUID execution-data keys skp:data:{guid} (no TTL) — minted per round-trip; cleaned by the
#         E2E net-zero teardown (L2KeysToCleanup). A leaked skp:data:* surfaces here as a redis SHA
#         mismatch (exit 1).
#       * the COMPOSITE backup key namespace corr:wf:proc:exec (skp:{corr}:{wf}:{proc}:{exec},
#         L2ProjectionKeys.CompositeBackup, BackupOptions 2-day TTL). NET-ZERO IS PROVEN BY ACTIVE
#         CLEANUP, NOT BY WAITING OUT THE TTL (D-07): the 2-day TTL CANNOT be waited out the way phase-39
#         drained its short-TTL keys. The E2E tests register EVERY composite they mint into
#         L2KeysToCleanup (and the Keeper INJECT/CLEANUP states actively delete it), so the composite is
...
#
# The net-zero discipline for the skp:data:* + composite corr:wf:proc:exec namespaces is enforced by the
# E2E teardown (the E2E tests register every namespace into L2KeysToCleanup); this gate's job is to
# SNAPSHOT-and-COMPARE the full unfiltered keyspace. There is deliberately NO destructive whole-db flush
# and NO prefix filter on the scan — widening to the full keyspace is what makes the composite part of the
# SHA. Likewise the skp-dlq-1 depth==0 check is a snapshot-and-compare assertion: the net-zero DLQ drain
# stays in the E2E teardown, NOT this gate (NO gate-side purge_queue) — so a teardown regression surfaces
# here as depth>0, not a silent pass.
#
# v4 SINGLE DLQ (D-07): the retired second DLQ (Phase 48) is gone. The sole surviving dead-letter queue is
# skp-dlq-1 (ConsolidatedErrorTransportFilter.Dlq1 — the consolidated transport-exhaustion forensic sink).
# The DLQ loop below is the SINGLE-element @('skp-dlq-1').
#
...
#
# CONTRACT-CHANGE NOTE (v4.0.0): the wire contract is a fresh BREAKING change (Pre/In/Post + Keeper 5-state
# recovery redesign). The live stack MUST run the REBUILT contract-changed images
...
#
# The full v4.0.0 stack MUST be up healthy (postgres, redis, rabbitmq, otel-collector, elasticsearch,
...
#        the single DLQ (skp-dlq-1) depth==0
...
try {
    Write-Host "Phase 49 close gate (triple-SHA) starting from $repoRoot" -ForegroundColor Cyan
    ...
    try {
        ...
        # => "Version": "3.5.0"). The phase-39 close-gate seed string was stale for v4 and is replaced here.
        $body = @{
    ...
    if (-not $procHealthy) {
        ...
        Write-Host "  (or the container predates the v4.0.0 wire contract — rebuild it: docker compose up -d --build processor-sample)." -ForegroundColor Red
        exit 2
    }
    Write-Host "  processor-sample is healthy (steady-state liveness key skp:$($procId.ToString().ToLower()) live)." -ForegroundColor Green

    # ---- Pre-flight: compose stack healthy (ONE canonical v4.0.0 list, processor-sample + keeper included) ----
    Write-Host "Pre-flight: compose stack health check..." -ForegroundColor Cyan
    # v4 canonical service list (verified against compose.yaml — the retired DLQ service is gone; keeper has deploy.replicas: 2).
    ...
    # ---- 3-GREEN cadence (D-06) — FULL suite, RealStack E2E run live ----
    $runResults = @()
    for ($i = 1; $i -le 3; $i++) {
    ...
    Write-Host "  Observed Phase 49 fact count: $($distinctPassed[0])." -ForegroundColor Gray

    # ---- Settle-drain the transient short-TTL round-trip keys ----
    # The short-TTL skp:data:* round-trip keys minted by the live RealStack E2E runs drain back to the
    # BEFORE baseline. Every RealStack E2E test STOPS its workflow in teardown, so no self-rescheduled cron
    # fire keeps minting fresh names after the runs finish. Wait for the keyspace to drain back to the
    # BEFORE set before snapshotting AFTER. This does NOT weaken leak detection: a key that never drains (a
    ...
    if ($settled) {
        ...
        Write-Host "  Settle timed out after 330s (skp:data:*=$dataN remain) — the AFTER SHA will flag the residual leak." -ForegroundColor Yellow
    }
    ...
    if ($beforeRedisHash -ne $afterRedisHash) {
        ...
        Write-Host "  OR a leaked COMPOSITE backup key skp:{corr}:{wf}:{proc}:{exec} (the E2E net-zero teardown" -ForegroundColor Red
        Write-Host "  should register every composite into L2KeysToCleanup; INJECT/CLEANUP actively delete it" -ForegroundColor Red
        Write-Host "  — the 2-day TTL is NOT waited out), or a churned processor-liveness key skp:{procId}" -ForegroundColor Red
        Write-Host "  (the seeded row's id changed across the run):" -ForegroundColor Red
    ...
    # ConsolidatedErrorTransportFilter.Dlq1 — the consolidated transport-exhaustion forensic sink; the second
    # DLQ was RETIRED in Phase 48). This uses `list_queues name messages` to read the depth column, kept OUT of
    # the name-SHA above (the SHA input stays `list_queues name`) so depth churn cannot break the name
    # invariant. The net-zero drain stays in the E2E teardown — NO gate-side purge_queue — so a teardown
    # regression surfaces here as depth>0.
    $depthRaw = docker exec sk-rabbitmq rabbitmqctl -q list_queues name messages | Out-String
    foreach ($q in @('skp-dlq-1')) {                          # ConsolidatedErrorTransportFilter.Dlq1 — sole surviving DLQ (the second DLQ was retired Phase 48)
    ...
    if (-not $allGood) {
        Write-Host "Phase 49 close gate FAILED. Resolve violations and re-run." -ForegroundColor Red
        exit 1
    }

    Write-Host "Phase 49 close gate PASSED." -ForegroundColor Green
    ...
    Write-Host "Operator: append these three SHA values + the Passed count + the skp-dlq-1 depth to .planning/phases/49-live-proof-close-gate/49-HUMAN-UAT.md Phase 49 GREEN-run record." -ForegroundColor Cyan
