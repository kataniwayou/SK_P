# Phase 28 close gate — v3.5.0 (triple-SHA)
...
#     Phase 28 adds two new skp: key families to the shared keyspace tests touch:
...
#     SHA-256, BEFORE = AFTER. Phase 28 adds the steady-state dispatch queue {procId:D} bound by
...
#
# Usage:
#   pwsh -File scripts/phase-28-close.ps1
#
# Exit codes:
...
try {
    Write-Host "Phase 28 close gate (triple-SHA) starting from $repoRoot" -ForegroundColor Cyan
    ...
    try {
        ...
        $body = @{
            ...
            version        = '3.5.0'
            description    = 'Phase 28 close-gate steady-state Processor.Sample row (genuine embedded hash, schema-less).'
    ...
    Write-Host "  Observed Phase 28 fact count: $($distinctPassed[0])." -ForegroundColor Gray
    ...
    if (-not $allGood) {
        Write-Host "Phase 28 close gate FAILED. Resolve violations and re-run." -ForegroundColor Red
        exit 1
    }

    Write-Host "Phase 28 close gate PASSED." -ForegroundColor Green
    ...
    Write-Host "Operator: append these three SHA values + the Passed count to .planning/STATE.md Phase 28 close entry." -ForegroundColor Cyan
