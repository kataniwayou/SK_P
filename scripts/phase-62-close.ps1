# Phase 62 close gate — v7.0.0 (triple-SHA)
# Per-Replica Processor Liveness & Self-Watchdog — live-proof + net-zero closeout
# ---------------------------------------------------------------------------
# This v7.0.0 gate is a verbatim clone of scripts/phase-58-close.ps1 (the proven triple-SHA protocol),
# carrying ONLY the D-09 redis-scan delta + the Phase 62 / v7.0.0 retitle: it reads BOTH embedded
...
#     The scan EXCLUDES the v7 per-replica liveness namespace by PREFIX (^skp:proc:) — see D-09 below —
#     so the v5 key families are still captured automatically in the BEFORE/AFTER SHA:
...
#       * (D-09) the v7 per-replica liveness namespace skp:proc:{procId:D} (the instance-index SET) +
#         skp:proc:{procId:D}:{instanceId} (the per-instance keys) — written by the LIVE processor-sample
#         replicas (2 healthy) AND the durably-unhealthy processor-badconfig replica on a sliding-TTL
#         heartbeat/startup loop. These are EXCLUDED from the redis name-SHA by the PREFIX filter
#         ^skp:proc: (replacing phase-58's single exact-key skp:{procId} exclusion). v7 instanceIds are
#         non-deterministic (pod/hostname/GUID-derived) and there are now 2-3 of them, so an exact single-key
#         match cannot exclude them; the ^skp:proc: prefix excludes BOTH families (SET + per-instance) for
#         ALL processors (healthy Sample x2 + unhealthy badconfig) in one filter. Their presence flaps with
#         the heartbeat-vs-TTL race and is briefly absent during the SC3 docker stop sk-redis outage, which
#         would otherwise churn the SHA. They are steady-state by intent (the live containers keep re-writing
#         them) — excluding them removes the timing fragility WITHOUT weakening leak detection for
#         skp:data:* / skp:msg:* (those do NOT start skp:proc:).
#         (D-09b NOTE for v7) processor-badconfig now DOES write a durable skp:proc:{badId}:{instanceId}
#         UNHEALTHY key + its index (the v7 startup orchestrator writes Unhealthy per iteration after
#         identity resolves — STATE-03), unlike v6 where it wrote nothing. The ^skp:proc: prefix excludes it
#         too, so it stays net-zero-harmless; it binds NO queue (Gate A withholds MarkHealthy), so it adds
#         NOTHING to the rabbitmq snapshot and needs NO exclusion there.
#   - rabbitmq: docker exec sk-rabbitmq rabbitmqctl -q list_queues name | sort | SHA-256, BEFORE == AFTER.
#     The steady-state dispatch queue {procId:D} bound by the live processor-sample replicas is
...
# procId across the whole 3-run gate, keeping its skp:proc:{procId:D} liveness namespace + {procId:D}
# dispatch queue in both redis/rmq snapshots).
#
...
# It binds NO queue and is INTENTIONALLY NOT a liveness pre-flight requirement (it never goes Healthy). In
# v7 it DOES write a durable skp:proc:{badId}:{instanceId} UNHEALTHY liveness key (STATE-03) — but that is
# under the ^skp:proc: prefix exclusion so it stays net-zero-harmless. It is seeded (schema + processor row)
# so the live E2E + the close-gate snapshots converge on the same rows, but it contributes NOTHING to the
# triple-SHA and is NOT awaited as healthy.
#
# CONTRACT-CHANGE NOTE (v7.0.0): the per-replica processor-liveness contract is a fresh BREAKING change
# (per-instance L2 keys skp:proc:{procId}:{instanceId} + instance-index SET, two-state health, dual-loop
# writer, in-memory L1, >=1-healthy orchestration gate, self-watchdog probe). The live stack MUST run the
# REBUILT contract-changed images, AND the processor-sample tier MUST come up as TWO replicas
# (deploy.replicas: 2), AND the new processor-badconfig container under the `badconfig` compose profile:
...
#
# The full v7.0.0 stack MUST be up healthy (postgres, redis, rabbitmq, otel-collector, elasticsearch,
# prometheus, orchestrator, processor-sample x2, baseapi-service, keeper). processor-badconfig is brought up
...
try {
    Write-Host "Phase 62 close gate (triple-SHA) starting from $repoRoot" -ForegroundColor Cyan
    ...
    # be healthy at pre-flight, and so its procId — hence its skp:proc:{procId:D} liveness namespace +
    # {procId:D} queue — is stable across the whole gate run (steady-state in both BEFORE and AFTER snapshots).
    ...
    # sentinel Names + definitions MUST match the E2E seed (62-02-SUMMARY) so the close-script and E2E rows
    # converge.
    ...
    # Sentinel definitions (MUST match Plan 02/03's E2E sentinels — 62-02-SUMMARY.md):
    ...
    function Get-OrCreateProcessorId([string]$name, [string]$sourceHash, $configSchemaId) {
        try {
            ...
            # Seed version: the verified live Processor.Sample value (src/Processor.Sample/appsettings.json
            ...
            $body = @{
                name           = $name
                version        = '3.5.0'
                description    = "Phase 62 close-gate seed row for $name (genuine embedded hash)."
    ...
    # Sample → compatible schema (Gate A passes → goes Healthy → steady-state liveness namespace + dispatch queue).
    $procId = Get-OrCreateProcessorId 'processor-sample' $sampleHash $compatibleSchemaId
    # BadConfig → clash schema (Gate A withholds MarkHealthy → durable Unhealthy liveness key, no queue → NOT awaited).
    ...
    # means the container only goes Healthy after this seed. processor-sample is a 2-replica service
    # (deploy.replicas: 2); the authoritative all-instances-healthy gate is the $services pre-flight loop
    # below — this wait checks the first replica as a fast fail-fast signal.
    ...
    while ((Get-Date) -lt $deadline) {
        # processor-sample emits NDJSON (one object per replica). Require ALL instances healthy.
        $instances = @(docker compose ps processor-sample --format json 2>$null |
            Where-Object { $_ -match '\S' } |
            ForEach-Object { $_ | ConvertFrom-Json })
        $h = if ($instances.Count -eq 0) { 'not-running' }
             else {
    ...
    if (-not $procHealthy) {
        ...
        Write-Host "  (or the container predates the v7.0.0 wire contract — rebuild it: docker compose up -d --build processor-sample)." -ForegroundColor Red
        exit 2
    }
    Write-Host "  processor-sample is healthy (steady-state liveness namespace skp:proc:$($procId.ToString().ToLower()) live)." -ForegroundColor Green

    # ---- Pre-flight: compose stack healthy (ONE canonical v7.0.0 list) ----
    # (D-09b) processor-badconfig is DELIBERATELY ABSENT from this health-required list: Gate A withholds its
    # MarkHealthy so it never reports Healthy, and it must NOT be a pre-flight requirement (its Docker
    ...
    # v7 canonical service list (verified against compose.yaml — keeper AND processor-sample have deploy.replicas: 2).
    ...
    foreach ($svc in $services) {
        ...
        # service (keeper has replicas: 2, processor-sample has replicas: 2) yields several lines, so the
        # concatenated string cannot be fed to a single ConvertFrom-Json — parse line-by-line and require
    ...
    Write-Host "  Observed Phase 62 fact count: $($distinctPassed[0])." -ForegroundColor Gray
    ...
    $afterRedis = (docker exec sk-redis redis-cli --scan | Where-Object { $_ -notmatch '^skp:proc:' } | Sort-Object -CaseSensitive | Out-String).Trim()
    ...
    if ($beforeRedisHash -ne $afterRedisHash) {
        ...
        Write-Host "  — the 300/600s SlotArrayOptions TTL is NOT waited out). The ^skp:proc: per-replica" -ForegroundColor Red
        Write-Host "  liveness namespace is EXCLUDED from this SHA, so a churned skp:proc: key cannot cause" -ForegroundColor Red
        Write-Host "  this mismatch — look at skp:data:* / skp:msg:* instead:" -ForegroundColor Red
    ...
    if (-not $allGood) {
        Write-Host "Phase 62 close gate FAILED. Resolve violations and re-run." -ForegroundColor Red
        exit 1
    }

    Write-Host "Phase 62 close gate PASSED." -ForegroundColor Green
