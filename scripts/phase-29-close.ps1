# Phase 29 close gate — v3.5.0 (triple-SHA)
# SourceHash identity + Processor.Sample + live execution round-trip closeout
# ---------------------------------------------------------------------------
# Identical protocol to scripts/phase-22-close.ps1 (the proven v3.4.0 triple-SHA gate),
# extended for the new steady-state processor-sample container:
...
#     Phase 28 added two new skp: key families to the shared keyspace tests touch:
#       * the steady-state processor-liveness key skp:{procId:D} — written by the LIVE
#         processor-sample container; it is STEADY-STATE (in BOTH the BEFORE and AFTER
#         snapshots), so it does not break the SHA as long as the procId is stable.
#       * the execution-data keys skp:data:{entryId:D} — minted per round-trip; cleaned by
#         the E2E net-zero teardown + the container's short ExecutionDataTtl (Plan 02/03),
#         NOT this script. A leaked skp:data:* surfaces here as a redis SHA mismatch (exit 1).
#   - TEST-RMQ-04/05 (D-09): docker exec sk-rabbitmq rabbitmqctl -q list_queues name | sort |
#     SHA-256, BEFORE = AFTER. Phase 28 added the steady-state dispatch queue {procId:D} bound by
#     the live processor-sample container — steady-state (both snapshots) as long as procId is stable.
...
# This is the SAME idempotent row the E2E (SampleRoundTripE2ETests) reuses, so the id never churns. A health
# exception was REJECTED — it would let the gate pass with a dead processor (defeats the liveness point).
#
# The full v3.5.0 stack MUST be up healthy (postgres, redis, rabbitmq, otel-collector,
# elasticsearch, prometheus, orchestrator, processor-sample, baseapi-service).
#
# Usage:
#   pwsh -File scripts/phase-29-close.ps1
#
# Exit codes:
...
try {
    Write-Host "Phase 29 close gate (triple-SHA) starting from $repoRoot" -ForegroundColor Cyan
    ...
    try {
        ...
        $body = @{
            ...
            version        = '3.5.0'
            description    = 'Phase 29 close-gate steady-state Processor.Sample row (genuine embedded hash, schema-less).'
    ...
    # ---- Pre-flight: compose stack healthy (ONE canonical v3.5.0 list, processor-sample included) ----
    ...
    Write-Host "  Observed Phase 29 fact count: $($distinctPassed[0])." -ForegroundColor Gray
    ...
    if ($beforeRedisHash -ne $afterRedisHash) {
        ...
        Write-Host "  Investigate a leaked skp: key. Likely a leaked execution-data key skp:data:* (the" -ForegroundColor Red
        Write-Host "  E2E net-zero teardown or the container ExecutionDataTtl — Plan 02/03), or a churned" -ForegroundColor Red
    ...
    if (-not $allGood) {
        Write-Host "Phase 29 close gate FAILED. Resolve violations and re-run." -ForegroundColor Red
        exit 1
    }

    Write-Host "Phase 29 close gate PASSED." -ForegroundColor Green
    ...
    Write-Host "Operator: append these three SHA values + the Passed count to .planning/STATE.md Phase 29 close entry." -ForegroundColor Cyan
