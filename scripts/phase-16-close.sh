#!/usr/bin/env bash
# Phase 16 close gate — v3.3.0 (Bash equivalent of scripts/phase-16-close.ps1)
...
#
# Usage:
#   bash scripts/phase-16-close.sh
#
# Exit codes:
...
echo "Phase 16 close gate starting from $REPO_ROOT"
...
if [[ -n "$NEW_MIGRATIONS" ]]; then
    ...
    echo "No EF migrations generated by Phase 16 (negative schema-push assertion HELD)."
fi
...
if [[ "$ALL_GOOD" != "1" ]]; then
    echo "Phase 16 close gate FAILED. Resolve violations and re-run." >&2
    exit 1
fi

echo "Phase 16 close gate PASSED."
echo "  Total facts GREEN: $PASSED_TOTAL"
echo "  psql \\l SHA-256:    $AFTER_PG_HASH"
echo "  redis-cli --scan SHA-256: $AFTER_REDIS_HASH"
echo "Operator: append these values to .planning/STATE.md Phase 16 P05 close entry."
