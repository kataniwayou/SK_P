# Phase 33 close gate — v3.7.0 (triple-SHA)
# Fault-recovery spike — pub/sub bind → unwrap → re-inject-by-type → flag[H] collapse closeout
# ---------------------------------------------------------------------------
# This v3.7.0 gate uses the protocol proven by scripts/phase-32.1-close.ps1 (its clone source),
...
#
# NOTE (Phase 33 — fault-recovery spike): the spike NEVER writes a no-TTL marker key (the Phase-32
# "cancelled circuit-breaker" was reverted before this milestone, so no such key is ever written). There is
# therefore NO marker scan-clean in this gate — the spike's poison/data/flag keys are all
# TTL'd/content-addressed and captured by the unfiltered --scan SHA + drained by the test's
# L2KeysToCleanup. The simpler proven triple-SHA protocol is the correct gate; the FaultRecoverySpikeE2ETests
# poison keys are net-zeroed by the TEST's L2KeysToCleanup (not this gate).
#
# The net-zero discipline for the skp:data:*/skp:flag:* namespaces is enforced by the E2E teardown
# (FaultRecoverySpikeE2ETests registers BOTH namespaces — plus every armed WRONGTYPE poison key — into
...
# full keyspace is what makes skp:flag:* part of the SHA.
#
...
# This is the SAME idempotent row the E2E (FaultRecoverySpikeE2ETests / IdempotentExactlyOnceE2ETests) reuses,
...
# orchestrator baseapi-service) — a mixed-version deployment mis-deserializes the new contract, AND the
# embedded SourceHash must match the host build (Pitfall 5) or the liveness gate false-passes/times out.
#
...
#
# Usage:
#   pwsh -File scripts/phase-33-close.ps1
#
# Exit codes:
...
try {
    Write-Host "Phase 33 close gate (triple-SHA) starting from $repoRoot" -ForegroundColor Cyan
    ...
    try {
        ...
        $body = @{
            ...
            version        = '3.7.0'
            description    = 'Phase 33 close-gate steady-state Processor.Sample row (genuine embedded hash, schema-less).'
    ...
    # ---- Pre-flight: compose stack healthy (ONE canonical v3.7.0 list, processor-sample included) ----
    ...
    Write-Host "  Observed Phase 33 fact count: $($distinctPassed[0])." -ForegroundColor Gray
    ...
    if ($beforeRedisHash -ne $afterRedisHash) {
        ...
        Write-Host "  teardown should drain BOTH namespaces + every armed WRONGTYPE poison — D-12), or a churned" -ForegroundColor Red
    ...
    if (-not $allGood) {
        Write-Host "Phase 33 close gate FAILED. Resolve violations and re-run." -ForegroundColor Red
        exit 1
    }

    Write-Host "Phase 33 close gate PASSED." -ForegroundColor Green
    ...
    Write-Host "Operator: append these three SHA values + the Passed count to .planning/STATE.md Phase 33 close entry." -ForegroundColor Cyan
